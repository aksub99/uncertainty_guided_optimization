...
if __name__ == '__main__':
    ...
    ensemble = [JTNNVAE_prop(vocab, hidden_size, latent_size, depthT, depthG, prop=args.property, dropout_rate_GRU=args.dropout_rate_GRU, dropout_rate_MLP=args.dropout_rate_MLP, drop_prop_NN=args.drop_prop_NN).cuda() for _ in range(ensembles)]
    ...
    for model in ensemble:
        ...
        schedulers.append(
            lr_scheduler.ExponentialLR(optimizers[-1], 0.9)
        )
    ...
    for epoch in tqdm.tqdm(range(MAX_EPOCH)):
        for i, model in enumerate(ensemble):
            ...
            word_acc = word_acc / (it + 1) * 100
            topo_acc = topo_acc / (it + 1) * 100
            assm_acc = assm_acc / (it + 1) * 100
            prop_acc = prop_acc / (it + 1)
            iter_step = it + 1
            ...
            schedulers[i].step()
            print("learning rate: %.6f" % schedulers[i].get_last_lr()[0])
        torch.save(ensemble.state_dict(), args.save_path + "/ensemble.finetuned_epoch-" + str(epoch))
    torch.save(ensemble.state_dict(), args.save_path + "/ensemble.finetuned_final")